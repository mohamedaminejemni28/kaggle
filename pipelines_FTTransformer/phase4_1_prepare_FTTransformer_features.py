...
def create_FTTransformer_feature_sets(
    train_file,
    output_file,
    ...
    """
    Create FTTransformer feature subsets using ANOVA F-score ranking.
    ...
    if results_df.empty:
        raise ValueError("No FTTransformer feature sets were created.")
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        results_df.to_excel(
            writer,
            sheet_name="FTTransformer_Feature_Sets",
            index=False,
        )

    logger.info(f"Saved FTTransformer Phase 4.1 results to: {output_file}")
...
def main():
    logger.info(">>>>>>>>>> Starting FTTransformer Phase 4.1: Preparing FTTransformer Feature Sets <<<<<<<<<<")

    config_path = os.path.join(PROJECT_ROOT, "configs", "config.yaml")
    config = read_yaml(config_path)

    group_column = config.get("SFS", {}).get("labels_column", "Group")
    participant_column = config.get("FTTransformer", {}).get("participant_column", "Participant")
    ...
    create_FTTransformer_feature_sets(
    ...
    logger.info(">>>>>>>>>> Completed FTTransformer Phase 4.1: Preparing FTTransformer Feature Sets <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in FTTransformer Phase 4.1: {e}")
        raise e
