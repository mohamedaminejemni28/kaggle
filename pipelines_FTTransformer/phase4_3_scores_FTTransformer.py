...
def build_sort_columns(df):
    """
    Choose sorting columns that exist in the FTTransformer result file.
    """
    preferred_sort = [
...
def add_readable_model_name(df):
    ...
    if "Name_Model" not in df.columns:
        ...
        for _, row in df.iterrows():
            ...
            d_token = config.get("d_token", "NA")
            n_heads = config.get("n_heads", "NA")
            n_layers = config.get("n_layers", "NA")
            lr = config.get("learning_rate", "NA")

            names.append(
                f"FTTransformer_Candidate_{candidate_id}_"
                f"{num_features}F_"
                f"DToken{d_token}_"
                f"Heads{n_heads}_"
                f"Layers{n_layers}_"
...
def select_top3_by_feature_count(df, feature_counts=None, top_n=3):
    ...
    for count in feature_counts:
        ...
        if subset.empty:
            logger.warning(f"No FTTransformer results found for {count} features.")
            continue
...
def save_excel_outputs(
    ...
    with pd.ExcelWriter(scores_file, engine="openpyxl") as writer:
        ranked_df.to_excel(writer, sheet_name="Ranked_All_FTTransformer", index=False)
    ...
    logger.info(f"Saved ranked FTTransformer scores to: {scores_file}")
    logger.info(f"Saved Top 3 FTTransformer scores to: {top3_file}")
...
def main():
    logger.info(">>>>>>>>>> Starting FTTransformer Phase 4.3: Selecting Best FTTransformer Scores <<<<<<<<<<")
    ...
    if not os.path.exists(input_file):
        raise FileNotFoundError(f"FTTransformer Phase 4.2 result file not found: {input_file}")

    logger.info(f"Reading FTTransformer Phase 4.2 results from: {input_file}")

    df = pd.read_excel(input_file)

    if df.empty:
        raise ValueError("FTTransformer Phase 4.2 results file is empty.")
    ...
    if missing_columns:
        raise ValueError(f"Missing required columns in FTTransformer results: {sorted(missing_columns)}")
    ...
    logger.info(f"Sorting FTTransformer models by: {sort_columns}")
    ...
    requested_feature_counts = FTTransformer_config.get("top3_feature_counts", [5, 6, 10])
    ...
    logger.info("Top 3 FTTransformer overall models:")
    ...
    logger.info(">>>>>>>>>> Completed FTTransformer Phase 4.3: Selecting Best FTTransformer Scores <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in FTTransformer Phase 4.3: {e}")
        raise e
