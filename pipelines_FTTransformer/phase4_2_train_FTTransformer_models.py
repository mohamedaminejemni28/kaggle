...
class GaitFTTransformer(nn.Module):
    def __init__(
        self,
        num_features,
        num_classes=2,
        ...
        self.feature_weight = nn.Parameter(torch.empty(num_features, d_token))
        self.feature_bias = nn.Parameter(torch.zeros(num_features, d_token))
...
def generate_FTTransformer_candidates(feature_sets_df, FTTransformer_config, seed=42):
    """
    Generate FTTransformer candidate models.
    Each row = one FTTransformer experiment.
    """
    random.seed(seed)
    np.random.seed(seed)

    feature_counts = FTTransformer_config.get("feature_counts", None)
    ...
    if feature_sets_df.empty:
        raise ValueError("No feature sets available for FTTransformer training.")

    d_token_list = FTTransformer_config.get("d_token", [16, 32, 64])
    n_heads_list = FTTransformer_config.get("n_heads", [2, 4])
    n_layers_list = FTTransformer_config.get("n_layers", [1, 2, 3])
    attention_dropout_list = FTTransformer_config.get("attention_dropout", [0.1, 0.2])
    ffn_dropout_list = FTTransformer_config.get("ffn_dropout", [0.1, 0.2])
    residual_dropout_list = FTTransformer_config.get("residual_dropout", [0.0, 0.1])
    learning_rate_list = FTTransformer_config.get("learning_rate", [0.001, 0.0005, 0.0001])
    batch_size_list = FTTransformer_config.get("batch_size", [4, 8, 16])
    weight_decay_list = FTTransformer_config.get("weight_decay", [0.0, 0.0001, 0.001])

    max_candidates = FTTransformer_config.get("max_candidates", 1000)
    ...
    for _, feature_row in feature_sets_df.iterrows():
        ...
        for d_token, n_heads, n_layers, attention_dropout, ffn_dropout, residual_dropout, lr, batch_size, weight_decay in itertools.product(
            d_token_list,
            n_heads_list,
            n_layers_list,
            attention_dropout_list,
            ffn_dropout_list,
            residual_dropout_list,
            learning_rate_list,
            batch_size_list,
            weight_decay_list,
        ):
...
def main():
    logger.info(">>>>>>>>>> Starting FTTransformer Phase 4.2: Training FTTransformer Models <<<<<<<<<<")
    ...
    FTTransformer_config = config.get("FTTransformer", {})

    seed = int(FTTransformer_config.get("random_state", 42))
    set_seed(seed)

    group_column = config.get("SFS", {}).get("labels_column", "Group")
    participant_column = FTTransformer_config.get("participant_column", "Participant")

    strict_subject_split = bool(FTTransformer_config.get("strict_subject_split", True))
    n_splits = int(FTTransformer_config.get("n_splits", 5))
    max_epochs = int(FTTransformer_config.get("max_epochs", 150))
    patience = int(FTTransformer_config.get("patience", 20))
    ...
    feature_sets_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["sfs_results_excel_file_FTTransformer"],
    )

    output_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["combination_results_file_FTTransformer"],
    )

    top3_output_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["scores_file_top3_FTTransformer"],
    )

    final_scores_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["scores_file_FTTransformer"],
    )

    checkpoint_dir = os.path.join(PROJECT_ROOT, "results", "FTTransformer_checkpoints")
    os.makedirs(checkpoint_dir, exist_ok=True)

    checkpoint_file = os.path.join(checkpoint_dir, "best_FTTransformer_model.pt")
    ...
    candidates_df = generate_FTTransformer_candidates(
        feature_sets_df=feature_sets_df,
        FTTransformer_config=FTTransformer_config,
        seed=seed,
    )

    logger.info(f"Number of FTTransformer candidates: {len(candidates_df)}")
    ...
    for _, candidate in candidates_df.iterrows():
        ...
        logger.info(
            ...
            f"d_token={model_config['d_token']} | "
            f"heads={model_config['n_heads']} | layers={model_config['n_layers']} | "
            f"dropout={model_config['attention_dropout']}/{model_config['ffn_dropout']} | "
    ...
    logger.info(f"Saved all FTTransformer results to: {output_file}")
    logger.info(f"Saved Top 3 FTTransformer results to: {top3_output_file}")
    ...
    logger.info(f"Best FTTransformer config: {best_config}")
    ...
    logger.info(f"Saved final FTTransformer scores to: {final_scores_file}")
    logger.info(f"Saved FTTransformer checkpoint to: {checkpoint_file}")
    ...
    if test_cm is not None:
        ...
        axes[0].set_title("FTTransformer Final Training Loss")
        ...
        ax.set_title("FTTransformer Final Training Loss")
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
        ax.grid(alpha=0.25)
    ...
    plot_file = os.path.join(PROJECT_ROOT, "results", "FTTransformer_final_training_plot.png")
    plt.savefig(plot_file)
    plt.close()

    logger.info(f"Saved FTTransformer final plot to: {plot_file}")

    logger.info(">>>>>>>>>> Completed FTTransformer Phase 4.2: Training FTTransformer Models <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in FTTransformer Phase 4.2: {e}")
        raise e
