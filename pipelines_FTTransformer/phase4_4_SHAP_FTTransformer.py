...
def save_importance_plot(summary_df, output_dir, top_n=15):
    ...
    plt.ylabel("Feature")
    plt.title("Top FTTransformer Feature Importance")
    plt.grid(axis="x", alpha=0.25)
    plt.tight_layout()

    plot_file = os.path.join(output_dir, "FTTransformer_permutation_importance_top_features.png")
...
def save_confusion_matrix_plot(cm, label_classes, output_dir):
    ...
    plt.title("FTTransformer Evaluation Confusion Matrix")
    ...
    plot_file = os.path.join(output_dir, "FTTransformer_evaluation_confusion_matrix.png")
    plt.savefig(plot_file, dpi=300)
    plt.close()
...
def main():
    logger.info(">>>>>>>>>> Starting FTTransformer Phase 4.4: FTTransformer Interpretation <<<<<<<<<<")
    ...
    n_repeats = int(FTTransformer_config.get("permutation_repeats", 30))
    ...
    checkpoint_file = os.path.join(
        PROJECT_ROOT,
        "results",
        "FTTransformer_checkpoints",
        "best_FTTransformer_model.pt",
    )

    output_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["output_file_FTTransformer"],
    )

    output_dir = os.path.join(
        PROJECT_ROOT,
        config["data"]["output_dir_FTTransformer"],
    )
    ...
    if top3_df.empty:
        raise ValueError("Top3 FTTransformer file is empty.")
    ...
    logger.info(f"Loaded best FTTransformer config: {model_config}")
    ...
    logger.info(f"Saved FTTransformer interpretation Excel to: {output_file}")
    ...
    logger.info("Top FTTransformer important features:")
    ...
    logger.info(">>>>>>>>>> Completed FTTransformer Phase 4.4: FTTransformer Interpretation <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in FTTransformer Phase 4.4: {e}")
        raise e
